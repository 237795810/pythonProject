...
@allure.title("{title}")
@allure.severity("critical")
@pytest.mark.parametrize(" task_id,title, agv_id, parameterse, template_name, code, message", get_test_data()["data14"])
...
@allure.title("{title}")
@allure.severity("critical")
@pytest.mark.parametrize("book_name, count, car_nums, agv_id1, agv_id, title, message", get_test_data()["data15"])
def test_location_setting_14(base_url, book_name, count, car_nums, agv_id1, agv_id, title, message):
    s = requests.session()
    api = Task_configuration(s, base_url)
    api.execution_template(book_name, count, car_nums, agv_id1)
    r = api.change_agv(agv_id)
    pytest.assume(str(message) in str(r.json()))
    allure.dynamic.title(title)
...
@allure.severity("critical")
@pytest.mark.parametrize("book_name, count, car_nums, agv_id1, agv_id, title, message", get_test_data()["data15"])
def test_location_setting_15(base_url, book_name, count, car_nums, agv_id1, agv_id, title, message):
    s = requests.session()
    api = Task_configuration(s, base_url)
    api.execution_template(book_name, count, car_nums, agv_id1)